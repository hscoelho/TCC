...
Porém, em situações em que não há conhecimento prévio do ambiente,
são necessários usar técnicas que realizam o mapeamento durante o
deslocamento do robô.

\section{Localização e Mapeamento Simultâneo (SLAM)}

\todo[inline]{refazendo...}

\subsection{Otimização de grafo de poses}

\subsection{Aplicações}
% Um campo de pesquisa importante na robótica móvel é o de localização
% e mapeamento simultâneo~(SLAM). Os primeiros algoritmos usavam
% métodos baseados em filtros de Bayes\cite{probabilistic_robots},
% porém aplicações modernas utilizam técnicas baseadas em grafos,
%  como detalhado em \textcite{graph_slam}.
