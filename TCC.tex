...
caso a condição	seja verdadeira ou falsa, respectivamente.
    ...
    { \color{red} colocar um exemplo de uma árvore de comportamento }
...
% TODO: Adicionar o NVIDIA ISAAC ROS e o StellaVSLAM(continuacao do OpenVSLAM
% nao estao no artigo do VSLAM feito pelo autor do Nav2, mas eles sao mais recentes
% e mais promissores(os outros nao foram atualizados faz tempo...)
...
que é composto por diversos pacotes menores, que definem as funcionalidades do robô Twil.
...
Todos pacotes modificados neste trabalho estão disponíveis repositório git
...
No pacote \textit{ufrgs\_map} está incluido uma representação em formato DWG desta
...
\textit{ufrgs\_gazebo}, junto com um arquivo \textit{world} do Gazebo,
...
Uma opção é utilizar sensores inercias, como o IMU.
Através de um \textit{plugin} do Gazebo, é possível simular um IMU no robô Twil e,
utilizando o pacote \textit{robot\_localization}, é possível combinar os dados
...
devido a recência das técnicas de VSLAM, não são tão robustos quanto os
pacotes anteriores.
...
Neste trabalho, será utilizada a planta do 1º andar do prédio Centenário da EE-UFRGS,
