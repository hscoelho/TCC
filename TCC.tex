...
% TODO: Dizer mais explicitamente vai ser feito e dar uma introdução dos próximos capítulos(1 frase)
...
% TODO: Arrumar/(retirar?) essa parte do DDS.
% "A parte da DDS continua com problemas. O RoS 2 não implementa a DDS. Ele
% simlesmente usa a DDS como midleware."
...
% TODO: Explicar que tópics são pub/sub e serviços são um mecanismo do tipo remote
% procedure call (RPC).
...
% TODO: Arrumar toda essa seção.
% 4) Tanto no ROS 1 quanto no ROS 2 as ações são implementadas em uma
% biblioteca própria (como quase todas as outras funcionalidades). E não
% são usados pelo Navigation 2 e pelo MoveiT!. Não mistura a explicação das
% ações com explicações sobre o Navigation 2 ou MoveIt. São coisas
% independentes, que até podem ser usadas juntas, mas são independentes.
...
% TODO: Incluir o pacote e e msg quando é falado sobre tipo de msg
% Na seção 3.3.1 (e nas outras) quando falas de tipos de mensgens, tens
% que incluir o pacote e o msg. oU seja usar algo como
% sensor_msgs/msg/JointState. Podem existir mensagens com o mesmo nome em
% pacotes diferentes ou como serviços.
% TODO: explicar o seguinte:
% Faltou explicar que odometria deve publicar a pose de base_link em
% relação a odom e que a localização deve publicar a pose de odom em
% relação a map. Tira a parte onde afirmas que odom deve ser usado como
% referência loca para curto prazo e map como referência global para longo
% prazo. A coisa é mais complicada do que isso.
