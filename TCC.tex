...
\todo[inline]{A imagem poderia ser na metodologia, pra explicar o que foi implementado no trabalho}
\begin{figure}[h]
    {
        \centering
...
\begin{itemize}
    ...
    % \item Controlador do robô que transforma comando de velocidade em 
    % comandos para as rodas do robô.
    % \item Descrição do robô?
\end{itemize}
...
\missingfigure{imagem do robo no rviz e gazebo}
...
\todo[inline]{
tenho que esclarecer melhor sobre o tcc antigo sobre localizacao

depois de implementar a localizacao e o odom aqui, 
eu vi que no tcc do rahul tinha sido implementado o odom do rtabmap e 
localizacao do amcl, mas o codigo que comecei mexendo nao tinha isso

tambem nao tinha o mundo do gazebo, e no trabalho antigo nao tinha nada sobre o
mundo do gazebo que foi utilizado, entao nao sei como o amcl funcionou

quando eu usei o amcl, eu desisti rapido porque achei que a planta do mapa e o 
mapa do gazebo tinham diferença suficiente pra localizacao nao ser muito boa
}
...
\todo[inline]{
    to pensando se deveria falar aqui sobre a configuração do robot localization,
    dizendo que usei a informacao de posicao mas seria melhor usar a velocidade da odometria
    e comoa mensagem da odometria ta com o sistema de coordenadas da velocidade diferente do
    esperado pelo robot localization
}
...
\todo[inline]{
    Traduzir e parafrasear esta explicação do funcionamento deste node do wiki do rtabmap:

    Using the feature correspondences between the images, 
    a RANSAC approach computes the transformation between 
...
\end{table}
\todo[inline]{
    quem sabe adicionar o tópico e tipo de mensagem na tabela?

    se eu adicionar os tópicos, posso referenciar esta tabela
    quando digo quais tópicos foram gravados no bag
}
...
\missingfigure{
    seria legal uma tabela com os sistemas de localizacao,
    igual/parecido ao da odometria
}
...
\todo[inline]{existe uma tradução para bags?}
...
\missingfigure[]{colocar screenshots do video do costmap com obstaculo}
